...
## D015：Dashboard 保留可见键盘焦点并隔离悬停样式

状态：已实现。

Dashboard 使用 2px 细竖线光标代替默认方块光标。不能完全隐藏光标，因为 `r`、`p`、`m`、`TAB` 和方向键都通过移动 point 表达当前键盘焦点；隐藏后按键仍会执行，却会让用户误以为没有反应。

Dashboard widget 默认会在鼠标停留期间持续高亮条目；同时使用 `r`、`p` 或 `m` 时，页面会出现鼠标 hover 与键盘 point 两个视觉焦点，让键盘移动看起来没有生效。当前通过 `dashboard-mode-hook` 只清除 Dashboard 按钮 overlay 的 `mouse-face`；手型指针、鼠标点击和按钮动作保持不变，键盘焦点则统一由细竖线表达。这个调整不修改插件源码，也不影响其他 buffer 的搜索、选区或高亮颜色。
