...
4. 保留 ~early-init.el~ 的早期职责，并让 ~init.el~ 成为只加载本地生成配置的最小引导入口。
5. 启动不再隐式联网安装包，并增加内置主题降级路径。
6. 增加离线验证脚本和可追踪回滚说明。
7. 确定 Literate 配置结构： ~config.org~ 是唯一人工维护主配置，受 Git 跟踪的 ~config.el~ 是 Tangle 产物。
...
- 运行状态文件不会被误提交。
- ~emacs --batch~ 无报错。
- 修改 ~config.org~ 后能够离线重新生成 ~config.el~ ，两者一致性由回归检查约束。
...
7. 使用 SIS + macism，让首次 GUI 和新顶层 GUI frame 默认进入 ABC 英文，忽略 child-frame，并保留后续手动中文选择。
...
- 使用 ~magit~ 建立核心 Git 工作台，并接入 ~project.el~ 。
- 配置 Status、全局 dispatch、当前文件 dispatch 与项目快捷入口。
- 配置窗口恢复、词级 diff、仓库发现和危险操作确认边界。
- 使用 Transient Posframe 把 GUI 命令菜单做成居中悬浮卡片，并保留 TTY 自动降级。
- 使用临时 Git 仓库回归验证延迟加载、Status 与 stage/unstage。
...
- 根说明和 ~docs/~ 已迁移为原生 Org 语法，并移除重复 Markdown 文档。
- 使用 ~org-modern~ 与 ~org-appear~ 建立可编辑的现代排版；保留 Batppuccin、17pt Fira Code / PingFang SC 和 Org 原生按键。
- 配置入口按 Literate Org 组织：人工编辑 ~config.org~ ，生成并跟踪 ~config.el~ ，由最小 ~init.el~ 离线加载。
...
- 配置修改能在 ~config.org~ 中同时说明设计原因与实现，并通过 Tangle 一致性检查避免生成物漂移。
