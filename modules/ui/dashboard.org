...
显示真实 Recent Files、project.el Projects 和 Bookmarks，并用整项高亮表达唯一键盘焦点。

* 边界

不伪造数据，不改 Dashboard 源码，不用自制首页替代成熟插件。

* 依赖

- 第三方 =dashboard= 包。
- =recentf= 、=project.el= 与 =bookmark.el=。
- =nerd-icons= 可选；缺失时使用文字列表。
...
- =r/p/m= 在 Recent Files、Projects 和 Bookmarks 区域循环选择。
- =TAB/S-TAB= 移动选择，=RET= 打开，=g= 刷新。
- 大写 =R/P/M= 按 Dashboard 默认契约反向移动。

* 验证

使用真实状态打开 Dashboard，验证键盘移动、整项高亮、鼠标不留第二高亮且仍可点击。

* 回滚

停用模块后 Emacs 回到普通 initial buffer；真实状态文件不删除。
